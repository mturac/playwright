...
class ResponseExtraInfoTracker {
  ...
  private _checkFinished(info: RequestInfo) {
    ...
    if (info.responses.length <= info.requestWillBeSentExtraInfo.length && info.responses.length <= info.responseReceivedExtraInfo.length) {
      // We have extra info for each request and response.
  ...
  private _fallbackToProvisionalHeaders(info: RequestInfo) {
    ...
    // forever.
    for (let i = 0; i < info.responses.length; ++i) {
      const response = info.responses[i];
      if (!(i in info.requestWillBeSentExtraInfo))
        response.request().setRawRequestHeaders(null);
      if (!(i in info.responseReceivedExtraInfo)) {
        response.setResponseHeadersSize(null);
        response.setRawResponseHeaders(null);
      }
    }
